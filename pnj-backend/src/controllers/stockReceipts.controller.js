...
const service               = require('../services/stockReceipt.service')
...
const create = asyncHandler(async (req, res) => {
  ...
  res.status(201).json(success(data, 'Penerimaan barang berhasil dicatat.'))
})

const update = asyncHandler(async (req, res) => {
  const data = await service.update(req.params.uuid, req.body, req.user)
  res.json(success(data, 'Penerimaan barang berhasil diperbarui.'))
})

const remove = asyncHandler(async (req, res) => {
  await service.remove(req.params.uuid)
  res.json(success(null, 'Penerimaan barang berhasil dihapus.'))
})
