...
Object.assign(openApiSpec.components.schemas, {
  SuratJalan: {
    ...
    properties: {
      ...
      sender_name: { type: 'string', nullable: true, maxLength: 255, example: 'PT Pengirim' },
      recipient_name: { type: 'string', nullable: true, maxLength: 255, example: 'PT Penerima' },
      lampiran_paths: {
        type: 'array',
        items: { type: 'string' },
  ...
  UpdateSuratJalanRequest: {
    ...
    properties: {
      ...
      internal_notes: { type: 'string', nullable: true },
      sender_name: { type: 'string', nullable: true, maxLength: 255 },
      recipient_name: { type: 'string', nullable: true, maxLength: 255 },
...
const sjQueryParams = [
  ...
  { name: 'project_code', in: 'query', schema: { type: 'string' } },
  { name: 'customer_uuid', in: 'query', schema: { type: 'string', format: 'uuid' } },
  { name: 'customer_name', in: 'query', schema: { type: 'string' } },
  { name: 'uuids', in: 'query', schema: { type: 'string' }, description: 'Comma-separated UUID Surat Jalan untuk export baris terpilih.' },
...
Object.assign(openApiSpec.components.schemas, {
  ...
  Invoice: {
    ...
    properties: {
      ...
      notes: { type: 'string', nullable: true },
      origin: { type: 'string', nullable: true, example: 'Gudang PNJ Pontianak' },
      destination: { type: 'string', nullable: true, example: 'Lokasi customer Kubu Raya' },
      cargo_description: { type: 'string', nullable: true, example: 'Material proyek' },
  ...
  CreateInvoiceRequest: {
    type: 'object',
    required: ['invoice_date', 'due_date', 'service_type'],
    properties: {
      project_uuid: { type: 'string', format: 'uuid' },
      ...
      service_type: { type: 'string', enum: ['delivery', 'rental', 'other'], example: 'delivery' },
      custom_service_name: { type: 'string', nullable: true, example: 'Jasa Bongkar Muat' },
      delivery_pricing_mode: { type: 'string', enum: ['shipment', 'item'], default: 'shipment', example: 'shipment' },
      ...
      destination: { type: 'string', nullable: true, example: 'Lokasi customer Kubu Raya' },
      cargo_description: { type: 'string', nullable: true, example: 'Material proyek' },
      items: { type: 'array', minItems: 0, items: { $ref: '#/components/schemas/InvoiceItemRequest' } },
...
Object.assign(openApiSpec.paths, {
  '/invoices': {
    ...
    post: {
      ...
      description: 'Role: super_admin atau admin_finance. Gunakan Idempotency-Key yang sama saat mengulang request dengan payload yang sama.',
      security: [{ bearerAuth: [] }],
      parameters: [{
        name: 'Idempotency-Key',
        in: 'header',
        required: false,
        description: 'UUID unik per percobaan create. Retry dengan key dan payload yang sama mengembalikan invoice yang sudah dibuat.',
        schema: { type: 'string', format: 'uuid' },
      }],
      requestBody: { required: true, content: { 'application/json': { schema: { $ref: '#/components/schemas/CreateInvoiceRequest' } } } },
      responses: {
        200: { description: 'Retry idempotent; invoice dari request sebelumnya dikembalikan.', headers: { 'Idempotency-Replayed': { schema: { type: 'string', example: 'true' } } }, content: { 'application/json': { schema: { $ref: '#/components/schemas/InvoiceSuccessResponse' } } } },
