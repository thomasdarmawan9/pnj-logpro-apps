...
const IMPORTS = [
  { sheetName: 'PT. OPTIMA DAYA SUKSES', customerName: 'PT. OPTIMA DAYA SUKSES' },
  { sheetName: 'CV. MULIA JAYA SAKTI', customerName: 'CV. MULIA JAYA SAKTI' },
  { sheetName: 'BAPAK HENGKY', customerName: 'BAPAK HENGKY' },
]
...
function applyPayment(record, row) {
  ...
  const transferredAmount = numericValue(row.getCell(7).value)

  if (!record.payment_date && paymentDate) record.payment_date = paymentDate
  if (!record.payment_date_text && paymentDateText) record.payment_date_text = paymentDateText
...
function readSheetRows(sheet) {
  ...
  sheet.eachRow((row, rowNumber) => {
    ...
    if (current.total_amount <= 0 && total > 0) {
      current.subtotal_amount = total
