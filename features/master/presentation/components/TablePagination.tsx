...
export default function TablePagination({ page, perPage, total, label, onPageChange, className = '', showBorderTop = true }: TablePaginationProps) {
  ...
  return (
    <div
      className={`flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-4 py-3 ${showBorderTop ? 'border-t' : ''} ${className}`}
    ...
    >
      ...
      <div className="flex items-center gap-1">
        ...
        >
          <ChevronsLeft size={14} style={{ color: 'var(--text-primary)' }} />
        </button>
        ...
        >
          <ChevronsRight size={14} style={{ color: 'var(--text-primary)' }} />
        </button>
      </div>
    </div>
  )
